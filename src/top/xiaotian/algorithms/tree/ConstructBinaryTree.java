...
//    return helpI(inorder, 0, inorder.length - 1, postorder, 0, postorder.length - 1);
    ...
    private TreeNode helpI(int[] inorder, int inSt, int inEn, int[] postorder, int postSt, int postEn) {
        ...
        for (int i = inSt; i <= inEn; i++) {
            ...
            if (inorder[i] == postorder[postEn]) {
                ...
                root.right = helpI(inorder, i + 1, inEn, postorder, postSt + (i - inSt - 1) + 1, postEn - 1);
                break;
            }
        }
    ...
    private TreeNode helpII(int[] preorder, int preSt, int preEn, int[] inorder, int inSt, int inEn) {
        ...
        Integer i = memoMap.get(preorder[preSt]);
        // i-inSt-1是根节点的左子树在inorder的偏移
        root.left = helpII(preorder, preSt + 1, preSt + 1 + (i - inSt - 1), inorder, inSt, i - 1);
        root.right = helpII(preorder, preSt + 1 + (i - inSt - 1) + 1, preEn, inorder, i + 1, inEn);
        return root;
    }
}
